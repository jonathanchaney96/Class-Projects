...
public class Game2048 extends JPanel {
        ...
        if (!myWin && !myLose) {
          switch (e.getKeyCode()) {
            case KeyEvent.VK_LEFT:
	            left();
	            break;
        // TODO: handle code for right, up, and down moves
  ...
  //move up
  public void up() {
    //TODO: same logic as right but handle up move
	  myTiles = rotate(90);
	    left();
	    myTiles = rotate(90);
  }
  ...
  boolean canMove() {
	// TODO: handle option to return true here 
	  ...
	  //check if 2 are the same next to each other
	  //tileAt()
	  for (int x=1; x< myTiles.length; x++) {
		  for(int y=1; y< myTiles.length; y++) {
			 Tile t = tileAt(x,y);
    ...
    // went through the whole board so can't move and return false
    return false;
  }
